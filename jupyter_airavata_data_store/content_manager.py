...
class AiaravataContentManager(ContentsManager):
    ...
    def file_exists(self, path =''):
        ...
        self.log.info("S3contents.GenericManager.file_exists: ('%s')", path)
        return self.fileOperations.file_exists(path)

    def dir_exists(self, path):
        # Does a directory exist at the given path?
        self.log.info("S3contents.GenericManager.dir_exists: path('%s')", path)
        return self.fileOperations.dir_exists(path)

    def get(self, path, content=True, type=None, format=None):
        # Get a file or directory model.
        self.log.info("S3contents.GenericManager.get] path('%s') type(%s) format(%s)", path, type, format)
        path = path.strip('/')
    ...
    def save(self, model, path):
        ...
        """
        self.log.info("S3contents.GenericManager: save %s: '%s'", model, path)
    ...
    def rename_file(self, old_path, new_path):
        ...
        """
        self.log.info("S3contents.GenericManager: Init rename of '%s' to '%s'", old_path, new_path)
        if self.file_exists(new_path) or self.dir_exists(new_path):
            ...
            self.log.info("S3contents.GenericManager: Actually renaming '%s' to '%s'", old_path,
                           new_path)
    ...
    def delete_file(self, path):
        ...
        """
        self.log.info("S3contents.GenericManager: delete_file '%s'", path)
    ...
    def is_hidden(self, path):
        ...
        """
        self.log.info("S3contents.GenericManager: is_hidden '%s'", path)
        return False
    ...
    def _get_directory(self, path, content=True, format=None):
        self.log.info("S3contents.GenericManager.get_directory: path('%s') content(%s) format(%s)", path, content, format)
        return self._directory_model_from_path(path, content=content)

    def _get_notebook(self, path, content=True, format=None):
        self.log.info("S3contents.GenericManager.get_notebook: path('%s') type(%s) format(%s)", path, content, format)
        return self._notebook_model_from_path(path, content=content, format=format)

    def _get_file(self, path, content=True, format=None):
        self.log.info("S3contents.GenericManager.get_file: path('%s') type(%s) format(%s)", path, content, format)
    ...
    def _directory_model_from_path(self, path, content=False):
        self.log.info("S3contents.GenericManager._directory_model_from_path: path('%s') type(%s)", path, content)
